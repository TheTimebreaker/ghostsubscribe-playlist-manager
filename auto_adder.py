...
class SettingsChannels(BaseModel):
    ...
    settings:Optional[PerChannelSettings] = None
class Settings(BaseModel):
    global_settings:SettingsGlobal
    channels:dict[str, SettingsChannels]

def grab_specific_setting(global_settings:SettingsGlobal, local_settings:Optional[PerChannelSettings], what_to_grab:str) -> Any:
    try:
        return getattr(local_settings, what_to_grab,
            getattr(global_settings, what_to_grab)
        )
...
def process(file:str, stop_event:Event) -> Generator[
    tuple[str, int, int],
    None,
    None
]:
    ...
    for i, (channel_id, channel_elem) in enumerate(data.channels.items()):
        ...
        try:
            ...
            if success:
                data.channels[channel_id].seen_video_ids = channel_seen_list
                write_settings(file, data)
        except youtube.SkippableException as error:
...
def create(filename:str, name:str, target_playlist_id:str, selector:ChannelUploadFilter) -> None:
    if os.path.isfile(f'auto_adder_config/{filename}'):
        raise youtube.SkippableException('Auto adder can\'t be created, the file already exists.')
