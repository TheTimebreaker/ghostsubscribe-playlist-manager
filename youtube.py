...
class UnskippableError(Exception):
    pass
...
class ResourceNotFoundError(SkippableError):
    pass


class InvalidValueError(SkippableError):
    pass


class QuotaError(UnskippableError):
    pass
...
def wrap_execute(request: Any) -> Any:
    ...
    def wrapped_execute(*args: Any, **kwargs: Any) -> Any:
        try:
            ...
            if (
                status == 403
                and """The request cannot be completed because you have exceeded your <a href="/youtube/v3/getting-started#quota">quota</a>."""
                in str(reason)
            ):
            ...
            if status == 404 and "The playlist identified with the request's <code>playlistId</code> parameter cannot be found." in str(reason):
                ...
                raise UnskippableError("Uncaught exception :(") from e
...
class Youtube:
    ...
    def _authorize(self, scopes: list[str]) -> Credentials:
        def get_new_creds() -> Credentials:
            ...
            creds: Credentials = flow.run_local_server(port=8080, access_type="offline", prompt="consent", timeout_seconds=120)
            return creds

        def get_valid_creds(creds: Credentials | None) -> Credentials:
        ...
        def read_creds() -> Credentials:
            creds: Credentials = Credentials.from_authorized_user_file(client_token_path, scopes)
            return creds
    ...
    def parse_any_url(self, url: str) -> Video | Playlist | Channel:
        ...
        raise SkippableError("No valid youtube URL.")
...
class Video(Youtube):
    ...
    def get_data(
        ...
        request = self.build.videos().list(part=",".join(part), fields=fields, id=self.id, maxResults=1)  # pylint:disable=no-member
        response = request.execute()
        data: dict = response["items"][0]
        return data
...
class Playlist(Youtube):
    ...
    def yield_elements(
        self,
        part: list[Literal["contentDetails", "snippet", "id", "status"]],
        fields: str | None = None,
    ) -> Generator[dict]:
    ...
    def add_video(self, video_id: str) -> bool:
        ...
        try:
            ...
            logging.error(
                "An error occurred while adding video %s to playlist %s: %s",
                video_id,
                self.id,
                error,
            )
    ...
    def get_video_playlist_id(self, video_id: str) -> str | Literal[False]:
        for video_element in self.yield_elements(["id", "snippet"]):
            ...
            if video_element["snippet"]["resourceId"]["videoId"] == video_id:
                return str(video_element["id"])
        return False

    def remove_video(self, video_id: str | None = None, video_playlist_id: str | None = None) -> bool:
        ...
        try:
            ...
            del_request = self.build.playlistItems().delete(id=video_playlist_id)  # pylint:disable=no-member
            del_request.execute()
            return True
        except AssertionError:
            logging.warning(
                "Video %s not found in playlist %s, there could not be removed.",
                video_id,
                self.id,
            )
        except HttpError as error:
            logging.error(
                "An error occurred while removing video %s from playlist %s: %s",
                video_id,
                self.id,
                error,
            )
    ...
    def verify(self) -> bool:
        try:
            ...
            request.execute()
            return True
        except (ResourceNotFoundError, InvalidValueError):
            return False
...
class Channel(Youtube):
    def __init__(self, channel_id: str) -> None:
        ...
        self.playlist_upload_id: str | None = None
    ...
    def _convert_handle_to_id(self, handle: str) -> str:
        ...
        raise UnskippableError(f"Some unknown BS happened while turning a Channel handle into a Channel ID. Response: {response}")
    ...
    def get_upload_playlist(
        ...
        if not self.playlist_upload_id:
            data = self.get_data(["contentDetails"], "items/contentDetails/relatedPlaylists/uploads")
            if not data:
        ...
        if full_videos_only:
            return Playlist(self.playlist_upload_id.replace("UU", "UULF", 1))
        if livestreams_only:
            return Playlist(self.playlist_upload_id.replace("UU", "UULV", 1))
        if shorts_only:
            return Playlist(self.playlist_upload_id.replace("UU", "UUSH", 1))
        return Playlist(self.playlist_upload_id)

    def get_profile_image(self, specific_size: int | bool = False) -> str:
        thumbnails: dict = self.get_data(part=["snippet"], fields="items/snippet/thumbnails")["items"][0]["snippet"]["thumbnails"]
    ...
    def list_uploads(
        ...
        for i, video_element in enumerate(
            p.yield_elements(
                part=["snippet"],
                fields="items/snippet/resourceId/videoId,prevPageToken,nextPageToken",
            )
        ):
...
def add_playlist_to_playlist(
    src_playlist_id: str | None = None,
    src_playlist: Playlist | None = None,
    target_playlist_id: str | None = None,
    target_playlist: Playlist | None = None,
) -> bool:
    ...
    success = True
    for video_element in src_playlist.yield_elements(
        part=["snippet"],
        fields="items/snippet/resourceId/videoId,prevPageToken,nextPageToken",
    ):
        video_id = video_element["snippet"]["resourceId"]["videoId"]
...
def add_channeluploads_to_playlist(
    src_channel_id: str | None = None,
    src_channel: Channel | None = None,
    target_playlist_id: str | None = None,
    target_playlist: Playlist | None = None,
    full_videos_only: bool = False,
    livestreams_only: bool = False,
